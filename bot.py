...
BOT_TOKEN = '8228285723:AAGx2iHSqJSQYcUL6AaL48vdIDIm86oUpuU'
...
start_processing_users = set()  # لمنع تكرار معالجة /start

# التحقق من المستخدم المسموح
def check_allowed_user(user_id):
    return user_id == ALLOWED_USER_ID

# زر البداية - مع إصلاح التكرار
...
async def start_handler(event):
    ...
    if user_id in start_processing_users:
        return
    start_processing_users.add(user_id)
    ...
    try:
        ...
        start_processing_users.discard(user_id)

# معالجة الأزرار - مع إصلاح التكرار
...
async def rotate_transfer_process(user_id, valid_accounts, target, original_event):
    """نقل الأعضاء بنظام التناوب بين الحسابات مع تأخير 30 ثانية"""
    try:
        total_transferred = 0
        total_failed = 0
        ...
        logger.info(f"Starting transfer process for user {user_id} with {len(valid_accounts)} accounts and {total_members} total members")
        
        # المتابعة حتى نقل جميع الأعضاء
        while members_processed < total_members and any(acc['active_members'] for acc in valid_accounts):
            for account_idx, account_data in enumerate(valid_accounts):
                # التحقق من توقف العملية
                active_processes = load_active_processes()
                if str(user_id) not in active_processes or active_processes[str(user_id)].get('status') == 'متوقف بواسطة المستخدم':
                    logger.info(f"Transfer process stopped for user {user_id}")
                    return
                
                if not account_data['active_members']:
                    continue
                
                if members_processed >= total_members:
                    break
                ...
                if result['success']:
                    total_transferred += 1
                    members_processed += 1
                    account_data['transferred'] += 1
                else:
                    total_failed += 1
                    account_data['failed'] += 1
                ...
                active_processes[str(user_id)]['accounts_status'][account_data['phone']]['transferred'] = account_data['transferred']
                active_processes[str(user_id)]['accounts_status'][account_data['phone']]['failed'] = account_data['failed']
                active_processes[str(user_id)]['last_update'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                save_active_processes(active_processes)
        ...
        try:
            ...
            # إذا لم نتمكن من تعديل الرسالة الأصلية، نرسل رسالة جديدة
            await original_event.reply(result_text)
        ...
        logger.info(f"Transfer process completed for user {user_id}: {total_transferred} successful, {total_failed} failed")
        ...
        try:
            await original_event.edit(f"❌ **حدث خطأ في عملية النقل:** {str(e)}")
        except:
            pass
    finally:
        processing_users.discard(user_id)
...
async def transfer_single_member(account_data, target, account_num):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Successfully transferred member {member.id} using account {account_data['phone']}")
            return {'success': True}
            
        except FloodWaitError as e:
            logger.warning(f"Flood wait for {account_data['phone']}: {e.seconds} seconds")
            # الانتظار للوقت المطلوب + 5 ثواني إضافية
...
async def show_live_stats(event):
    ...
    stats_text += f"🔄 **آخر تحديث:** {process_data.get('last_update', 'غير متوفر')}\n\n"
    ...
    buttons = [
        [Button.inline("🔄 تحديث الإحصائيات", "refresh_stats")],

...
BOT_TOKEN = '8052900952:AAGCbBxSw5drwFg-lxsg905wNZejZzfSWsQ'
...
async def start_process(event):
    ...
    for account in accounts:
        try:
            ...
            # استخدام event loop منفصل لكل عميل
            ...
            participants = await client.get_participants(source_entity, limit=10)  # تحديد الحد
            ...
            for participant in participants:
                try:
                    if not participant.bot:  # تجنب البوتات
                        await client(InviteToChannelRequest(target_entity, [participant]))
                        success_count += 1
                        await asyncio.sleep(5)  # تأخير أطول لتجنب الحظر
...
async def start_web_server():
    ...
    # استخدام المنفذ من متغير البيئة أو 8080 افتراضيًا
...
async def main():
    ...
    try:
        await bot.send_message(-1002142959124, "🤖 البوت يعمل الآن بنجاح!")
    except:
        pass
...
if __name__ == '__main__':
    # استخدام asyncio.run بدلاً من loop
    asyncio.run(main())

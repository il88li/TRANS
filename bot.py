...
class TelegramBot:
    ...
    def setup_handlers(self):
        ...
        self.application.add_handler(CallbackQueryHandler(self.register_handler, pattern="^register_"))
    ...
    async def message_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id in user_states:
            ...
            if state == "waiting_message":
                ...
                try:
                    # إنشاء عميل جديد
                    app = Client(f"user_{user_id}", api_id=API_ID, api_hash=API_HASH)
                    await app.connect()
                    
                    # طلب كود التحقق
                    sent_code = await app.send_code(phone_number)
                    context.user_data['phone_code_hash'] = sent_code.phone_code_hash
                    context.user_data['client'] = app
                    
                    await update.message.reply_text(
                        "تم إرسال كود التحقق إلى هاتفك. أرسل الكود الآن:"
                    )
                    
                except Exception as e:
                    await update.message.reply_text(f"خطأ في تسجيل الحساب: {str(e)}")
                    del user_states[user_id]
                ...
                try:
                    ...
                    self.data_manager.save_data(USERS_FILE, users)
    ...
    async def schedule_publishing(self, user_id: int, process: dict):
        ...
        self.scheduler.add_job(publish_job, trigger, id=f"publish_{user_id}")

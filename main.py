...
async def save_contact(user, user_client):
    try:
        ...
        if not phone:
            ...
            phone = f"+999{user.id:08d}"  # رقم افتراضي باستخدام ID المستخدم
        ...
        if "PHONE_NUMBER_INVALID" in error_msg:
            # إذا فشل بسبب الرقم، نحاول بطريقة بديلة
            try:
                # محاولة إضافة المستخدم كجهة اتصال بدون رقم
                from telethon.tl.functions.contacts import AddContactRequest
                from telethon.tl.types import InputPhoneContact
                
                # استخدام رقم افتراضي مختلف
                phone = f"+000{user.id:08d}"
                contact = InputPhoneContact(
                    client_id=user.id,
                    phone=phone,

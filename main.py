...
async def save_contact(user, user_client):
    try:
        ...
        return False, f"خطأ في حفظ الجهة: {str(e)}"
...
@client.on(events.CallbackQuery(data=b"settings_main"))
...
@client.on(events.CallbackQuery(data=b"account_settings"))
...
async def timing_settings_handler(event):
    ...
    buttons = [
        ...
        [Button.inline(f"الفاصل الحالي: {current_delay} ثانية", b"no_action")],
        [Button.inline(f"الحد الحالي: {current_limit} عضو", b"no_action")],
        [Button.inline("🔙 رجوع للإعدادات", b"settings_main")]
    ]
...
@client.on(events.CallbackQuery(data=b"set_delay"))
...
@client.on(events.CallbackQuery(data=b"set_limit"))
...
@client.on(events.CallbackQuery(data=b"check_members"))
...
• الحالة: {progress[4]}
    ...
    await event.edit(status_text, buttons=[[Button.inline("🔙 رجوع", b"settings_main")]])
...
@client.on(events.CallbackQuery(data=b"settings_main"))
async def back_to_settings_main(event):
    await settings_main_handler(event)
        ...
        try:
            ...
            for i, member in enumerate(valid_members):
                ...
                try:
                    ...
                    if contact_saved:
                        saved_contacts += 1
                        contact_msg = "✅ تم حفظ جهة الاتصال"
                    else:
                        contact_msg = f"❌ {contact_reason}"

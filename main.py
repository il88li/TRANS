...
async def transfer_members(user_id, event):
    try:
        ...
        try:
            ...
            # الحصول على قائمة الأعضاء - لا تحتاج إلى صلاحيات مشرف
            await event.edit("🔍 جاري جلب الأعضاء من المصدر...")
            
            # محاولة جلب الأعضاء بطريقة متدرجة
            all_members = []
            offset = 0
            limit = 200
            ...
            while True:
                try:
                    members = await user_client.get_participants(source_entity, limit=limit, offset=offset)
                    if not members:
                        break
                    ...
                    # إذا كان العدد قليلاً نكسر الحلقة
            ...
            for i, member in enumerate(valid_members):
                ...
                if i % 5 == 0 or i == len(valid_members) - 1:
                    progress_msg = f"""
📊 تقدم الإضافة:
• المضافة: {added_count}
• الفاشلة: {failed_count}
• المتبقية: {len(valid_members) - i - 1}
• النسبة: {((i+1)/len(valid_members)*100):.1f}%
                    """
...
async def main():
    # بدء مهمة keep-alive في الخلفية

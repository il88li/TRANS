...
async def start_handler(event):
    ...
    await event.reply(
        ...
        "• لا تحتاج إلى أن تكون مشرفاً في المجموعة المصدر",
        buttons=buttons
    )
...
async def more_options_handler(event):
    ...
    await event.edit("**الإعدادات المتقدمة:**\n\n✅ لا تحتاج إلى أن تكون مشرفاً في المجموعة المصدر", buttons=buttons)
...
async def set_source_handler(event):
    ...
    await event.edit("📥 أرسل معرف المجموعة/القناة المصدر:\n\n❗ **ملاحظة:** لا تحتاج إلى أن تكون مشرفاً في المصدر")
...
async def back_main_handler(event):
    ...
    await event.edit("**مرحباً بك في بوت نقل الأعضاء**\n\n✅ لا تحتاج إلى أن تكون مشرفاً في المجموعة المصدر", buttons=buttons)
...
async def transfer_members_individual(user_id, event):
    try:
        settings = get_user_settings(user_id)
        if not settings:
        ...
        try:
            ...
            # الحصول على قائمة الأعضاء باستخدام iter_participants بدلاً من get_participants
            await event.edit("🔍 جاري جلب الأعضاء من المصدر...")
            
            all_members = []
            try:
                # استخدام iter_participants الذي لا يحتاج إلى offset
                async for member in user_client.iter_participants(source_entity, aggressive=True):
                    all_members.append(member)
                    if len(all_members) % 100 == 0:
            ...
            update_progress(user_id, total_members=total_members, processed=0, failed=0, status='جاري النقل')
            
            await event.edit(f"🔍 تم العثور على {total_members} عضو صالح من أصل {len(all_members)}\n🚀 بدء عملية النقل الفردي...")
            ...
            for i, member in enumerate(valid_members):
                ...
                try:
                    ...
                    if await is_user_in_group(member.id, target_entity, user_client):
                        await event.edit(f"⏩ العضو {i+1}/{total_members} موجود بالفعل في الهدف، تخطي...")
                        continue
                    
                    # حفظ العضو كجهة اتصال
                    ...
                    if await add_to_group(member.id, target_entity, user_client):
                        added_count += 1
                        status_msg = f"✅ تمت إضافة العضو {i+1}/{total_members}"
                    else:
                        failed_count += 1
                        status_msg = f"❌ فشل إضافة العضو {i+1}/{total_members}"
                    
                    # تحديث التقدم
                    update_progress(user_id, processed=i+1, failed=failed_count, status='جاري النقل')
                    
                    # تحديث الحالة كل 5 أعضاء أو عند الأعضاء المهمة
                    if (i + 1) % 5 == 0 or (i + 1) == total_members:
                        progress_msg = f"""
📊 تقدم النقل الفردي:

• المعالج: {i+1}/{total_members}
• المضافة: {added_count}
• الفاشلة: {failed_count}
• المحفوظة: {saved_contacts}
• النسبة: {((i+1)/total_members*100):.1f}%
...
• العدد الإجمالي: {len(all_members)}
...
🎯 **طريقة العمل:**
1. البحث عن عضو غير موجود في الهدف
2. حفظه كجهة اتصال
3. إضافته إلى الهدف
4. التكرار مع العضو التالي
            """
